...
import (
	...
	"github.com/schwarz/inventoryservice/receipt/model"
	"io"
	"log"
	"net/http"
	"os"
...
func handleReceipts(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		receiptList, err := model.GetReceipts()
		if err != nil {
	...
	case http.MethodPost:
		...
		defer file.Close()
		f, err := os.OpenFile(filepath.Join(model.ReceiptDir, handler.Filename), os.O_WRONLY|os.O_CREATE, 0666)
		defer f.Close()
		io.Copy(f, file)
...
func handleDownload(w http.ResponseWriter, r *http.Request) {
	...
	file, err := os.Open(filepath.Join(model.ReceiptDir, fileName))
	defer file.Close()
	if err != nil {
